import { Match3Mode } from './type.ts';
 ...
 */
const BLOCKS_DEFINE: Record<Match3Mode | 'special', string[]> = {
  /** Test mode piece set */
...
export function match3GetConfig(
  customConfig: Partial<Match3Config> = {}
): Match3Config {
...
export function match3GetBlocks(mode: Match3Mode): string[] {
  return [...BLOCKS_DEFINE[mode], ...BLOCKS_DEFINE.special];
}

//* get all block types in the game and divide into 2 different types of common and special
export function getAllBlockTypesInGame(mode: Match3Mode) {
  return { common: BLOCKS_DEFINE[mode], special: BLOCKS_DEFINE.special };
}
