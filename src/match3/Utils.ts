...
import { MatchFormat, Match3Type, Match3Grid, Match3Position } from './type';
 ...
 * @param types List of mattypes avaliable to fill up slots
 * @returns A 2D array filled up with types
 */
export function match3CreateGrid(
  rows = 6,
  columns = 6,
  types: Match3Type[],
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      // const isSpecialType = specialTypes ? Math.random() > 0.85 : false;
      const isSpecialType = true;

      let type =
        isSpecialType && specialTypes
          ? match3GetRandomType(specialTypes)
          : match3GetRandomType(types);
 ...
 */
export function match3GetRandomType(
  types: Match3Type[],
  exclude?: Match3Type[]
) {
  let list = [...types];
 ...
 */
export function match3SetPieceType(
  grid: Match3Grid,
  position: Match3Position,
  type: number
) {
 ...
 */
export function match3IsValidPosition(
  grid: Match3Grid,
  position: Match3Position
) {
...
//? find all match in the board ds
/**
 ...
 * @returns
 */
export function match3GetMatchesByOrientation(
  puzzlingBoard: PuzzlingBoard,
  matchSize: number,
  orientation: Match3Orientation,
  matchFormat: MatchFormat = 'regular'
) {
  const grid = puzzlingBoard.board.pieceGrid;
  const commonTypes = puzzlingBoard.board.commonTypes;

  const matches = [];
  const rows = grid.length;
  const columns = grid[0].length;
  let lastType = undefined;
  let currentMatch: (Match3Position & { type: Match3Type })[] = [];
  ...
  // console.log('commontypes length', commonTypes?.length);
  ...
  for (let p = 0; p < primary; p++) {
    for (let s = 0; s < secondary; s++) {
      ...
      if (type && type === lastType) {
        ...
        // currentMatch.push({ row, column, type });
        if (
          (matchFormat === 'regular' && type < commonTypes?.length + 1) ||
          (matchFormat === 'special' && type >= commonTypes?.length + 1) ||
          matchFormat === 'both'
        ) {
          // console.log('special', { row, column, type });
          currentMatch.push({ row, column, type });
        }
      } else {
...
//  * @param grid The grid to be searched
//  * @param matchSize The size of the match (usually 3)
//  * @param direction If the search is horizontal or vertical
//  * @returns
//  */
// export const getAllMatchedsByDirection = (
//   grid: Match3Grid,
//   matchSize: number,
//   direction: Match3Orientation
// ) => {
//   const matches = [];
//   const rows = grid.length;
//   const columns = grid[0].length;
...
//   // Define primary and secondary orientations for the loop
//   const primary = direction === 'horizontal' ? rows : columns;
//   const secondary = direction === 'horizontal' ? columns : rows;

//   for (let p = 0; p < primary; p++) {
//     for (let s = 0; s < secondary; s++) {
//       // On horizontal 'p' is row and 's' is column, vertical is opposite
//       const row = direction === 'horizontal' ? p : s;
//       const column = direction === 'horizontal' ? s : p;
//       const type = grid[row][column];
 ...
 */
export function match3IncludesPosition(
 ...
 */
export function match3GetMatches(
  puzzlingBoard: PuzzlingBoard,
  // filter?: Match3Position[],
  matchFormat: MatchFormat = 'both',
  matchSize = 3
) {
  const allMatches = [
    ...match3GetMatchesByOrientation(
      puzzlingBoard,
      matchSize,
      'horizontal',
      matchFormat
...
export function getChangesInPositionToApplyGravity(match3: PuzzlingBoard) {
  const { rows, columns } = match3.config;

  const changes: Match3Position[][] = [];

  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      const currentPosition = { row: r, column: c };
      const belowPosition = { row: r + 1, column: c };

      const currentPeiceType = match3.board.getTypeByPosition(currentPosition);
      let belowPeiceType = match3.board.getTypeByPosition(belowPosition);

      if (currentPeiceType != 0 && belowPeiceType === 0) {
        let emptyPieceCount = 0;
        const emptyPositionChain = [belowPosition];

        while (belowPeiceType === 0) {
          emptyPieceCount++;
          emptyPositionChain.push({
            row: emptyPositionChain[emptyPositionChain?.length - 1].row + 1,
            column: c,
          });
          belowPeiceType = match3.board.getTypeByPosition(
            emptyPositionChain?.[emptyPositionChain?.length - 1]
          );
        }

        let aboveEmptyRow = currentPosition.row;

        while (aboveEmptyRow >= 0) {
          const oldPosition = {
            row: aboveEmptyRow,
            column: c,
          };
          ...
          const newPosition = {
            row: aboveEmptyRow + emptyPieceCount,
            column: c,
          };
          changes.push([oldPosition, newPosition]);
          match3SwapPieces(match3.board.pieceGrid, oldPosition, newPosition);
...
export function fillToEmptySpaceOnPieceGrid(
  grid: Match3Grid,
  ...
  // Create a temp grid that will provide pieces to fill up corresponding slots
  // using the same grid creation algorithm to avoid pre-made combinations
  const newRandomPieceGrid = match3CreateGrid(
    grid.length,
    grid[0].length,
    commonTypes,
    specialTypes
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      if (!grid[r][c]) {
        grid[r][c] = newRandomPieceGrid[r][c];
        newPositions.push({ row: r, column: c });
      }
    }
  }
